...
for l in lines:
    ...
    if 'if (node.body) node.body->accept(static_cast<ASTVisitor&>(*this));' in l:
        # Check if we are inside ForEach loop's else branch
        if len(out) > 0 and 'sm.enterExistingScope(node.bodyScopeId);' in out[-1]:
